...
@click.option('--demos', is_flag=True, prompt='Serving demo repos', help="Are you serving demo repos?")
...
def write_wheels_dot_env(d, for_local=False):
    for repo_name, remote in pfsc_conf.REMOTE_WHEELS.items():
        var_name = f'PYO_WHL_{repo_name.upper().replace("-", "_")}'
        if remote and (pfsc_conf.USE_REMOTE_WHEELS_IN_LOCAL_ENV or not for_local):
            value = remote
        else:
            value = pfsc_conf.LOCAL_WHEELS[repo_name]
        d[var_name] = value
