...
def pfsc_server(deploy_dir_path, mode, flask_config, tag='latest',
    ...
    d = {
        ...
        'volumes': [
            f'{PFSC_ROOT}/{direc}:/proofscape/{direc}'
...
def proofscape_oca(deploy_dir_path, tag='latest', mount_code=False, mount_pkg=None):
    ...
    if mount_code:
        ...
        d['volumes'].append(f'{PFSC_ROOT}/src/pfsc-ise/dist/ise.bundle.js:/home/pfsc/proofscape/src/pfsc-server/static/ise.bundle.js:ro')
        d['volumes'].append(f'{PFSC_ROOT}/src/pfsc-pdf/build/generic:/home/pfsc/proofscape/src/pfsc-server/static/pdfjs:ro')

        for name in ["pfsc-util", "displaylang", "sympy", "pfsc-examp"]:
            whl = conf.LOCAL_WHEELS[name]
            d['volumes'].append(
                f'{PFSC_ROOT}/src/{name}/dist/{whl}:/home/pfsc/proofscape/src/pfsc-server/static/whl/{whl}:ro'
            )
...
def nginx(deploy_dir_path, tag=conf.NGINX_IMAGE_TAG,
    ...
    if not dummy:
        ...
        d['volumes'].extend([
            ...
            f'{PFSC_ROOT}/src/pfsc-ise/dist:/usr/share/nginx/ise:ro',
            ...
            f'{PFSC_ROOT}/src/pyodide:/usr/share/nginx/pyodide:ro',
        ])
        for name, whl in conf.LOCAL_WHEELS.items():
            if conf.REMOTE_WHEELS.get(name) is None:
                d['volumes'].append(
                    f'{PFSC_ROOT}/src/{name}/dist/{whl}:/usr/share/nginx/whl/{whl}:ro'
                )
